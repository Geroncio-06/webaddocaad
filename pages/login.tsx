...
export default (_props: any) => {
  ...
  async function login() {
    ...
    try{
      ...
      setLoading(true);
      await firebaseClient.auth().signInWithEmailAndPassword(email, pass2);
      ...
      var metaData = await firebaseClient.database().ref('Usuário/'+iD);
          ...
          if(data.funcao != null){localStorage.setItem('cargo',data.funcao);} else if(data.funcao == null){localStorage.setItem('cargo','Cargo');};
          ...
          if(data.telefone != null){localStorage.setItem('phone',data.telefone);} else if(data.telefone == null){localStorage.setItem('phone','(xx) 9 xxxx-xxxx');};
          if(data.nascimento != null){localStorage.setItem('calendar',data.nascimento);} else if(data.nascimento == null){localStorage.setItem('calendar','dd/mm/aaaa');};
          if(data.batizadonasaguas != null){localStorage.setItem('batizadoaguas',data.batizadonasaguas);} else if(data.batizadonasaguas == null){localStorage.setItem('batizadoaguas','');};
          if(data.batizadonoespirito != null){localStorage.setItem('batizadonoespirito',data.batizadonoespirito);} else if(data.batizadonoespirito == null){localStorage.setItem('batizadonoespirito','');};
          if(data.endereco != null){localStorage.setItem('endereco',data.endereco);} else if(data.endereco == null){localStorage.setItem('endereco','Endereco');};
          if(data.bairro != null){localStorage.setItem('bairro',data.bairro);} else if(data.bairro == null){localStorage.setItem('bairro','Bairro');};
          ...
          if(data.password != null){localStorage.setItem('password',data.password);} else if(data.password == null){localStorage.setItem('password','password');};
...
<form className={styles.overview_panel_start}>

  <div>
    <h1 className={styles.text_input_cadastrar}>Seja Bem Vindo</h1>
    <h2 className={styles.text_input_cadastrar2}>O AD Doca foi pensado para você, otimizando a 
    sua experiência</h2>
    <h3 className={styles.text_input_cadastrar2}>com o cadastro online.</h3>
  </div>

  <Link href="/account">
    <button className={styles.heading_cadastrar}>
        <div className={styles.text_button}>CRIAR CONTA</div>
    </button>
  </Link>
</form>
...
<div className={styles.overview_panel}>
  ...
  <Link href="/account">
    <button className={styles.heading_cadastrar2}>
        <div className={styles.text_button}>CRIAR CONTA</div>
    </button>
  </Link>
